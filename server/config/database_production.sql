-- Group Expense Tracker Production Database Schema
-- Enhanced MariaDB Setup Script for Production Deployment
...
GRANT SELECT, INSERT, UPDATE, DELETE ON expense_tracker_prod.* TO 'expense_tracker_user'@'localhost';
GRANT SHOW VIEW ON expense_tracker_prod.* TO 'expense_tracker_user'@'localhost';
FLUSH PRIVILEGES;
...
-- Insert sample data (remove for production)
-- Sample user with bcrypt hash (password: 'Demo123!')
...
('Team Lunch', 85.50, 'expense', 'Food', 1, 1, CURDATE()),
('Monthly Salary', 3500.00, 'income', 'Salary', 1, NULL, CURDATE()),
('Office Supplies', 45.75, 'expense', 'Office', 1, 1, CURDATE() - INTERVAL 1 DAY),
('Freelance Project', 750.00, 'income', 'Freelance', 1, NULL, CURDATE() - INTERVAL 2 DAYS);
...
SELECT 'Production database schema is ready for deployment!' as final_status;
